...
- Builds inventories (flights/trains/hotels/restaurants/activities) like v2
- Only references users by user_id/handle in the output (no repeating core profile fields)

Output → data/travel_groups_integrated_v3.json
"""
...
# Interest → destination bias (to keep it realistic & user-driven)
INTEREST_CITY_HINTS = {
    "mountains": ["Interlaken","Zermatt","Innsbruck","Lucerne"],
    "lakes": ["Lucerne","Interlaken","Geneva"],
    "museums": ["Paris","Amsterdam","Berlin"],
    "old towns": ["Prague","Florence","Bern"],
    "beaches": ["Nice","Malaga","Lisbon"],
}
...
def nearest_airport_for_city(city):
    for loc in LOCATION_DATA:
        if loc["city"] == city:
            return random.choice(loc["airports"]) if loc["airports"] else None
    return None
...
# ============================
# Inventory Generators (no user duplication in output)
# ============================

def gen_flight_offers_for_user(user, meet_city, depart_date, return_date):
    origin_city = user.get("home_base", {}).get("city")
...
# ============================
# Draft plan composer (uses user + matchmaker; output references users only)
# ============================

def choose_destinations_by_interests(users_list):
    # Find common or popular interests and bias destination accordingly
    bag = []
    for u in users_list:
        for it in u.get("interests", [])[:5]:
            bag.append(it)
    top = random.sample(bag, k=min(3, len(bag))) if bag else []
    pools = []
    for it in top:
        pools.extend(INTEREST_CITY_HINTS.get(it, []))
    if not pools:
...
def compose_draft_plan(member_users, member_mm, itinerary_style="multi-stop"):
    # Meeting city = first destination (common pattern)
    ...
    # pick a diet from any member who set one; else none
    group_diet = None
    for u in member_users:
        diet = ((u.get("diet_health") or {}).get("diet"))
        if diet and diet != "none":
            group_diet = diet; break
    group_diet = group_diet or "none"

    # any member's accommodation price band to bias hotels
    pbands = []
    for u in member_users:
        acc = u.get("accommodation") or {}
        if acc.get("price_band"): pbands.append(acc["price_band"])
    price_band = random.choice(pbands) if pbands else "mid-range"

    # aggregate interests
    all_interests = sorted({i for u in member_users for i in (u.get("interests") or [])}) or ["museums","old towns"]
    ...
    # Flights by member (only user refs in output)
    flights_by_user = {}
    for u in member_users:
...
# ============================
# Group assembly (NO duplication of user data in output)
# ============================
...
def build_trip_context_from_mm(seed_mm):
    # Derive rules from matchmaker profile (meeting preference, detail level, weights)
    ...
    # objective via weight emphasis
    cw = (mp.get("compatibility_weights") or {})
    # simple heuristic: if pace/comfort are high → comfort; if budget high → cost; else time
    ...
    return {
        ...
        "destinations": [],  # filled by composer via interests
        "date_window": {
...
def main():
    ...
    mm_profiles = json.load(open(MM_PATH, "r", encoding="utf-8"))
    ...
    for g in range(N_GROUPS):
        ...
        trip_ctx = build_trip_context_from_mm(member_mm[0])

        # Compose plan & inventories (this function reads data but will NOT duplicate it in the output)
        ...
        group_doc = {
            ...
            # Only references to users (no repetition of core data)
            "members": [{
                "user_id": u["user_id"],
                "rovermitra_handle": (u.get("contacts") or {}).get("rovermitra_handle", f"rm_{u['user_id'][:6]}")
            } for u in member_users],
        ...
        if random.random() > RICH_GROUP_RATIO:
            # Lightly degrade output_preferences and luggage to create variety
