...
app = FastAPI(title="RoverMitra Llama API", version="2.0.0")
...
def root():
    return {
        "service": "RoverMitra Llama API Server",
        "version": "2.0.0",
        "status": "running",
        "public_url": ngrok_public_url,
        "endpoints": {
            "matches": "POST /api/matches-bulk",
            "health": "GET /health",
            "pool": "GET /pool",
            "rank": "POST /rank",
            "prefilter": "POST /prefilter",
            "docs": "GET /docs"
        }
    }
...
async def find_companions(request: MatchRequest):
    """
    Main endpoint to find travel companions.
    Accepts a query_user dict and returns ranked matches.
    """
    try:
        ...
        print(f"🚀 Finding matches for: {query_user.get('name', 'Unknown')}")
        
        # Step 1: AI Prefilter using BGE embeddings
        print(f"🔍 Prefiltering from {len(_pool_data)} candidates...")
        
        if _EMB and _cached_ids is not None and _cached_embs is not None:
            # Build query embedding from bio, interests, and values
            query_parts = []
            if query_user.get('bio'):
                query_parts.append(query_user['bio'])
            if query_user.get('interests'):
                query_parts.append(' '.join(query_user['interests']))
            if query_user.get('values'):
                query_parts.append(' '.join(query_user['values']))
            
            query_text = ' '.join(query_parts) if query_parts else "travel companion"
            query_emb = _EMB.encode([query_text], normalize_embeddings=True)[0]
            
            # Compute similarities with cached embeddings
            similarities = np.dot(_cached_embs, query_emb)
            
            # Get top candidates
            min_k = max(request.min_k, int(len(_pool_data) * request.prefilter_percent))
            top_k_filter = min(min_k, len(_pool_data))
            top_indices = np.argsort(similarities)[-top_k_filter:][::-1]
            
            shortlist = [_pool_data[i] for i in top_indices]
            print(f"✅ Prefiltered to {len(shortlist)} candidates using embeddings")
        else:
            # Fallback: use first N candidates
            shortlist = _pool_data[:min(100, len(_pool_data))]
            print(f"⚠️ No embeddings available, using top {len(shortlist)} candidates")
        
        # Step 2: Build LLM prompt
        print("📝 Building LLM prompt...")
        prompt = build_llm_prompt(query_user, shortlist[:50], top_k=request.top_k)  # Limit to 50 to avoid token overflow
        
        # Step 3: Get LLM ranking
        print("🤖 Running LLM ranking...")
        llm_output = get_llama_ranking(prompt, max_new_tokens=800, temperature=0.0)
        ...
        try:
            # Try to extract JSON from output (handle cases where model adds extra text)
            ...
            print(f"✅ Found {len(formatted_matches)} matches for {query_user.get('name', 'Unknown')}")
        ...
        print(f"❌ Error in matches-bulk: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/rank")
async def rank_candidates(request: RankRequest):
    """Rank candidates using loaded Llama model (low-level endpoint)"""
    try:
...
if __name__ == "__main__":
    ...
    print("\n" + "="*80)
    print("🚀 RoverMitra Llama API is LIVE!")
    print("="*80)
    ...
    print(f"📊 Pool Data:         GET  {ngrok_public_url}/pool")
    print("="*80)
